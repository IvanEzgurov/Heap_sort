...
public class Main {
    ...
    // Рекурсивный алгоритм heapify-down. Узел с индексом `i` и два его потомка нарушают свойства кучи
    private static void heapify(int[] A, int i, int size) {
        ...
        // поменять местами с потомком имеющим большее значение и вызовите heapify-down для дочернего элемента
        if (largest != i) {
            swap(A, i, largest);
    ...
    public static void heapsort(int[] A) {
        ...
        //несколько раз излекаем из кучи, пока она не станет пустой
        while (n > 0) {
            A[n - 1] = pop(A, n);
            n--;
